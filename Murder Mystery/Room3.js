class Story3Room3 {
  ...
  setup() {
    ...
    this.img2 = loadImage("https://upload.wikimedia.org/wikipedia/commons/6/6c/US_state_abbrev_map.png"
    );
  ...
  infobuttonRoom1(infobuttonX, infobuttonY) {
    ...
    if (distance < 40 / 2) {
      fill("white");
      rect(70, 60, 300, 120, 4, 4, 4, 4);
      fill("black");
      text(
        "A map reveals a haunting pattern of recent murders matching the modus operandi of the latest victim. Delving into a list of past incidents, you must decode an image the cunning murderer seeks to convey. The past holds the key to interpreting the present, and your detective skills are put to the test as you connect the dots on the map",
        77,
        70,
        290,
        200
...
sound() {
  ...
  hint(hintX, hintY) {
    ...
    if (this.hintBox == true) {
      fill("white");
      rect(-200, -10, 200, 40, 4, 4, 4, 4);
      fill("black");
      text(
        "The figure you get after connecting\n the dots is a joyful emotion",
        -195,
        5
      );
    }
  ...
  hoverstates() {
    if (mouseX > 370 && mouseX < 470 && mouseY > 90 && mouseY < 200) {
      push();
      scale(1.8);
      this.states(230, 20);
      pop();
    }
    if (mouseX > 480 && mouseX < 680 && mouseY > 90 && mouseY < 200) {
      push();
      scale(1.8);
      this.mapp(230, 20);
      pop();
